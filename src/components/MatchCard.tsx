...
const isFav = (favorites: string[] | undefined, name: string) => !!favorites?.some((f) => name.includes(f));
...
export function MatchCard({ match, league, favoriteTeams }: Props) {
  const [open, setOpen] = useState(false);
  const [detail, setDetail] = useState<DetailState>({ status: "idle" });

  const favorite = isFav(favoriteTeams, match.home.name) || isFav(favoriteTeams, match.away.name);
